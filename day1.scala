val input = """kjrqmzv9mmtxhgvsevenhvq7
              ...
              |xlkdlhlk23four
              |8ninejseven5""".stripMargin

// Step 1
input
  .split('\n')
  .map { line =>
    val firstDigit = line.find(_.isDigit).get.toString.toInt
    val lastDigit = line.findLast(_.isDigit).get.toString.toInt
    val result = firstDigit * 10 + lastDigit
    println(s"$line: $firstDigit $lastDigit $result")
    result
  }
  .sum

// Step 2
val mappings: Map[String, Int] = Map(
  "one" -> 1,
  "1" -> 1,
  "two" -> 2,
  "2" -> 2,
...
input
  ...
  .map { line =>
    ...
    val (_, _, first, last) = mappings.foldLeft((line.length, -1, 0, 0)) { case ((firstMinIndex, lastMaxIndex, firstDigit, lastDigit), (key, value)) =>
      val firstOccurence = line.indexOf(key)
      val (minIndex, fDigit) = if (firstOccurence != -1 && firstOccurence < firstMinIndex) {
        (firstOccurence, value)
      } else {
        (firstMinIndex, firstDigit)
      }

      val lastOccurence = line.lastIndexOf(key)
      val (maxIndex, lDigit) = if (lastOccurence != -1 && lastOccurence > lastMaxIndex) {
        (lastOccurence, value)
      } else {
        (lastMaxIndex, lastDigit)
      }

      (minIndex, maxIndex, fDigit, lDigit)
    }
